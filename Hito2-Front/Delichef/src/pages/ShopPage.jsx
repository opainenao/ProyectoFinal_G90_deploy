...
const ShopPage = ({ navigate }) => {
  const [kits, setKits] = useState([]); // Ahora kits es un estado, no la constante KITS
...
const cleanCategory = (categoryName) => {
        ...
        // 1. Convertir a mayúsculas para manejar inconsistencias (opcional, pero buena práctica)
        let cleaned = categoryName.toUpperCase();
        
        // 2. Remover los sufijos (manteniendo el orden de búsqueda más específico al más genérico)
        cleaned = cleaned.replace(' PREMIUM', '');
        cleaned = cleaned.replace(' GOURMET', '');
        cleaned = cleaned.replace(' OPCIONAL', '');
        
        // 3. Devolver la primera letra en mayúscula y el resto en minúscula
